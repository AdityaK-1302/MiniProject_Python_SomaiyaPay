...
def login():
    ...
    Button(
        root,
        text="Enter",
        font=("BankGothic Lt BT", 15, "bold"),
    ).place(x=600, y=470)
    Button(root,
           text="Clear",
           font=("BankGothic Lt BT", 15, "bold"),
           command=clear_login).place(x=700, y=470)
...
def register():
    ...
    def check_credentials():
        ...
        if pass_check(password):
            Label(root,
                  ...
                  font=("BankGothic Lt BT", 15),
                  fg="green").place(x=30, y=580)
            Button(root,
                   text="Proceed ->",
                   font=("BankGothic Lt BT", 18, "bold")).place(x=650, y=550)
        else:
            messagebox.showerror(
                "ERROR",
                "Enter a valid password. Conditions:\n1.Lenght minimum 8 characters\n"
    ...
    Button(root,
           text="Submit",
           font=("BankGothic Lt BT", 15, "bold"),
           command=check_credentials).place(x=30, y=530)
    Button(root,
           text="Clear",
           font=("BankGothic Lt BT", 15, "bold"),
           command=clear_register).place(x=150, y=530)
    name = name_info.get()
    rollno = rollno_info.get()
    email = email_info.get()
    password = pass_info.get()
...
text1 = Label(text="Welcome to SomaiyaPay",
              bg='Red',
              fg='Black',
              padx=233,
              pady=4,
...
Button(root,
       text="Register",
       font=("BankGothic Lt BT", 18, "bold"),
       command=register).place(x=250, y=300)
Button(root,
       text="Log In",
       font=("BankGothic Lt BT", 18, "bold"),
       command=login).place(x=650, y=300)
